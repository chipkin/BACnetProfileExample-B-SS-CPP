...
richer profile — see the series table in [README.md](README.md).
...
4. Confirm the services you do **not** implement are still rejected - for B-SS,
   WriteProperty to any object.
...
```bash
python tools/gen-objects-properties.py BACnetProfileExample-B-SS-CPP            # rewrite
python tools/gen-objects-properties.py BACnetProfileExample-B-SS-CPP --check    # fail if stale
```
